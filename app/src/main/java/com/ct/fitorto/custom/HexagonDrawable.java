...
public class HexagonDrawable extends Drawable {
    ...
    private Path createHexagon(int size, int centerX, int centerY) {
        final float section = (float) (2.0 * PI / SIDES);
        int radius = size / 2;
        Path hex = temporal;
        hex.reset();
        hex.moveTo((float)(centerX + radius * cos(0)),(float)(centerY + radius * sin(0)));

        for (int i = 1; i < SIDES; i++) {
            hex.lineTo((float)(centerX + radius * cos(section * i)),(float)(centerY + radius * sin(section * i)));
        }
